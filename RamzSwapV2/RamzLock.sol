...
contract RamzLock is IRamzLock {
        ...
        returns (address)
    {
        ...
        try IRamzSwapPair(token).factory() returns (address factory) {
    ...
    function _isValidLpToken(address token, address factory)
        ...
        returns (bool)
    {
        IRamzSwapPair pair = IRamzSwapPair(token);
        address factoryPair = IRamzSwapFactory(factory).getPair(
            pair.token0(),
            pair.token1()
        );
